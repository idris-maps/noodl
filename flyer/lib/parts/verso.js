...
function col1(x,y,conf, callback) {
	menuPage(conf.data.menu[0],x,y,conf,0, function(str0) {
		menuPage(conf.data.menu[1],x,y,conf,850, function(str1) {
			menuPage(conf.data.menu[7],x,y,conf,1550, function(str2) {
...
function col2(x,y,conf,callback) {
	menuPage(noodleSoups(conf.data.menu),x,y,conf,620, function(str0) {
		var str1 = logo(conf.data.logo, conf.c, 'translate(150,30) scale(1.2)')
		callback(str0 + str1)
	})
}

function col3(x,y,conf,callback) {
	menuPage(rices(conf.data.menu),x,y,conf,730, function(str0) {
		menuPage(conf.data.menu[4],x,y,conf,0, function(str1) {
			callback(str0 + str1)
		})
	})
}
